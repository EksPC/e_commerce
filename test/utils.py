...
def get_stock_log() -> list[dict]:
    return requests.get(f"{STOCK_URL}/stock/logs").json()
...
def order_test(message: str) -> requests.Response:
    return requests.get(f"{ORDER_URL}/orders/test/kafka")
...
def stock_outbox():
    return requests.get(f"{STOCK_URL}/stock/outbox")
...
def test_slow_checkout_saga():
    ...
    add_credit_to_user(user["user_id"], 1000)
    user = find_user(user["user_id"])
    print(f"Testing slow checkout saga for user: {user['user_id']} with credit: {user['credit']}")
    ...
    print("Simulating slow processing by waiting for 30 seconds before checkout...")
...
@dataclass
class StartPaymentCommandPayload:
    """
